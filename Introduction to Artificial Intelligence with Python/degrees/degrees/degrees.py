...
def shortest_path(source, target):
    ...
    while queueFrontier.empty() == False:
        ...
        for movie, person in neighbors:
            if not queueFrontier.contains_state(person) and person not in explored:
                childNode = Node(state= person, parent=currentNode, action=movie)

                if(childNode.state == target):
                    return createPath(childNode)
                
                queueFrontier.add(childNode)    
